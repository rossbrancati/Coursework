...
def choose_word(hand, N=100):
    ...
    bestscore = 0
    bestword = None
    #generate a list of possible lengths of words
    word_len_list = list(range(calculate_handlen(hand)))
    #convert the hand to a list
    ls = hand_to_list(hand)
    #loop over nummber of simulations
    for i in range(N):
        #convert the hand to a list
        ls = hand_to_list(hand)
        #randomly select a number of letters from the word
        word_len = random.choice(word_len_list)
        #Randomly sample a set of possible number of letters in the hand
        sample = random.sample(ls, word_len)
        #transform word into a list
        word = "".join(sample)
        #check for a word in the words list that contains the randomly selected letters
        if ps2.is_valid_word(word, hand, word_list):
            score = ps2.get_word_score(word, calculate_handlen(hand))
            #check if the score is better then the first score
            if score > bestscore:
                #override the bestscore
                bestscore = score
                #and switch the best word to the current word
                bestword = word
...
def play_mc_hand(hand, N=100):
    ...
    while calculate_handlen(hand)>0:
        #get the bestword and bestscore of the current hand
        bestword, bestscore = choose_word(hand, N)
        if bestword is not None:
            #append the list of the best words
            wordls.append(bestword)
            #add bestscore to the score of the hand
            handscore = handscore + bestscore
            #update the hand for next iteration of the loop
            hand = ps2.update_hand(hand, bestword)
        else:
            break
...
def play_n_mc_hand(hand, N, n):
    ...
    for i in range(n):
        #call the play_mc_hand function
        wordls, handscore = play_mc_hand(hand, N=100)
        #append the mc_hands list with the wordls 
        mc_hands.append(wordls)
        #append the mc_scores with the score for that particular word
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while handlen > 0:
        ...
        if word == '!!':
            ...
            #call the choose_word function
            bestword, bestscore = choose_word(hand, N=100)
            #Print the best word and its score
