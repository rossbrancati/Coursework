...
def get_val_metrics(x_train, y_train, k, clf):
    ...
    for train_index, test_index in kf.split(x_train, y_train):
        ...
        #append the results dictionary with the evaluation scores
        results['Precision'].append(metrics.precision_score(y_val_true, validation_preds, average='weighted'))
        results['Recall'].append(metrics.recall_score(y_val_true, validation_preds, average='weighted'))
        results['F1_Score'].append(metrics.f1_score(y_val_true, validation_preds, average='weighted'))
        
        #Note: I could not get the ROC_AUC score to work. It seems like the labels need
        #to be one hot encoded for this to work, but given that this is extra credit and my limited
        #time, I decided to evaluate my best model based on the other metrics
        #results['AUC'].append(metrics.roc_auc_score(y_val_true, validation_preds, average='weighted', multi_class='ovr'))
    ...
    #Had same issue as before when trying to calculate AUC_ROC score
    ...
    return [[validation_precision, validation_recall, validation_f1_score], [training_precision, training_recall, training_f1_score]]
...
def main():
    ...
    #clf = LogisticRegression(random_state=0, max_iter=10000, C=1)
    ...
    test_precision = metrics.precision_score(test_y, y_preds, average='weighted')
    test_recall = metrics.recall_score(test_y, y_preds, average='weighted')
    test_f1_score = metrics.f1_score(test_y, y_preds, average='weighted')
    #test_auc = metrics.roc_auc_score(test_y, y_preds)
    #and print the metrics
    print('Evaluation Metrics for Gene Expression Test Set:')
    print("Test Precision:", test_precision)
    print("Test Recall:", test_recall)
    print("Test F1 Score:", test_f1_score)
    #print("Test ROC_AUC:", test_auc)
    print('cross_validation script for Q3.2 completed')
